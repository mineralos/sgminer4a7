...
#ifndef JOB_FLIP_BYTES
    // header
    memcpy(jobdata + 2, work->data, BLOCK_HEADER_LEN);

    // target
//    memcpy(jobdata + 2 + BLOCK_HEADER_LEN, work->target, TARGET_LEN);
    ...
    // end nonce
    memset(jobdata + 2 + BLOCK_HEADER_LEN + TARGET_LEN, 0xff, 4);
#else
    // header
    for(i = 0; i < (BLOCK_HEADER_LEN >> 2); i++)
    {
        jobdata[2 + i * 4 + 0] = work->data[4 * i + 3];
        jobdata[2 + i * 4 + 1] = work->data[4 * i + 2];
        jobdata[2 + i * 4 + 2] = work->data[4 * i + 1];
    ...
    // end nonce
    jobdata[(BLOCK_HEADER_LEN + TARGET_LEN + 2) + 0] = 0xff;
    jobdata[(BLOCK_HEADER_LEN + TARGET_LEN + 2) + 1] = 0xff;
    jobdata[(BLOCK_HEADER_LEN + TARGET_LEN + 2) + 2] = 0xff;
...
//    memset(tmp_buf, 0, sizeof(tmp_buf));
    int crc_size = JOB_LENGTH - 2;
    for(i = 0; i < crc_size / 2; i++)
    {
        tmp_buf[(2 * i) + 1] = jobdata[(2 * i) + 0];
        tmp_buf[(2 * i) + 0] = jobdata[(2 * i) + 1];
    }
    crc = CRC16_2(tmp_buf, crc_size);
    jobdata[JOB_LENGTH - 2] = (uint8_t)((crc >> 8) & 0xff);
    jobdata[JOB_LENGTH - 1] = (uint8_t)((crc) & 0xff);

    memcpy(job, jobdata, JOB_LENGTH);

#if 0
    printf("[create job]\n");
    char *htarget = bin2hex(work->target, 32);
    applog(LOG_DEBUG, "**********target: %s", htarget);
    free(htarget);
...
{
    applog(LOG_NOTICE, "disable chip %d in chain %d", chip_id, a1->chain_id);
...
#else
    if(inno_cmd_read_result(a1->chain_id, ADDR_BROADCAST, buffer, NONCE_LEN))
#endif
    {
        *job_id = buffer[0] >> 4;
        *chip_id = buffer[1];
...
//        applog(LOG_WARNING, "Got nonce for chain %d / chip %d / job_id %d", a1->chain_id, *chip_id, *job_id);
//        applog(LOG_WARNING, "Nonce: %02x %02x %02x %02x", nonce[0], nonce[1], nonce[2], nonce[3]);
...
bool set_chain_pll(uint8_t chain_id, int pll_lv)
{
    ...
    while(!inno_cmd_write_register(chain_id, ADDR_BROADCAST, reg, REG_LENGTH))
    {
        usleep(50000);
        timeout--;
        if(timeout == 0) 
...
bool inno_check_voltage(struct A1_chain *a1, int chip_id, inno_reg_ctrl_t *s_reg_ctrl)
{
    ...
    if (!inno_cmd_read_register(a1->chain_id, chip_id, reg, REG_LENGTH)) {
        ...
        if((tmp_v > CHIP_VOL_MAX) || (tmp_v < CHIP_VOL_MIN)){
            nVolTotal++;
            applog(LOG_ERR,"chain %d chip %d voltage up to %.3f", a1->chain_id, chip_id, tmp_v);
        }
