...
const struct PLL_Clock PLL_Clk_12Mhz_33[119]={
...
const struct PLL_Clock PLL_Clk_12Mhz_57[359]={
    {  0,   30, A4_PLL(1,  80, 5)},
    {  1,   32, A4_PLL(1,  84, 5)},
    {  2,   33, A4_PLL(1,  88, 5)},
    {  3,   35, A4_PLL(1,  92, 5)},
...
int A1_ConfigA1PLLClock(int optPll)
{
    ...
    if(optPll>0){
        pllIdx=0;
        if(optPll<=PLL_Clk_12Mhz_33[0].speedMHz) 
		{
            pllIdx=0; //found
        }else{
            for(i=1; i < A7_PLL_LV_NUM; i++){
                if((optPll<PLL_Clk_12Mhz_33[i].speedMHz)&&(optPll>=PLL_Clk_12Mhz_33[i-1].speedMHz)){
                    pllIdx=i-1; //found
                    break;
                }
            }
        ...
        applog(LOG_NOTICE, "OptPLL =%d, index=%d, A1 PLL Clock = %dMHz",pllIdx,PLL_Clk_12Mhz_33[pllIdx].speedMHz);
    }
...
extern const uint8_t default_reg_33[119][12];
...
//    assert((pllClkIdx > 0) && (pllClkIdx < A5_PLL_CLOCK_MAX));
	...
	memcpy(temp_reg, default_reg_33[pllClkIdx], REG_LENGTH);
	...
	if (chip_id) {
		applog(LOG_WARNING, "A1 %d chip %d PLL set to %d %d MHz", cid, chip_id,
		       pllClkIdx, PLL_Clk_12Mhz_33[pllClkIdx].speedMHz);
	} else {
